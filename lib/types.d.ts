...
export declare type ActivityConfig<TContext> = (ctx: TContext, activity: ActivityDefinition<TContext>) => DisposeActivityFunction | void;
export declare type Activity<TContext> = string | ActivityDefinition<TContext>;
export interface ActivityDefinition<TContext> extends ActionObject<TContext> {
    id: string;
    type: string;
}
export declare type InvokeCreator<TFinalContext, TContext> = (ctx: TContext, event: EventObject) => Promise<TFinalContext>;
export interface InvokeDefinition<TContext, TEvent extends EventObject> extends ActivityDefinition<TContext> {
    /**
...
export declare type DelayedTransitions<TContext, TEvent extends EventObject> = Record<string | number, string | TransitionConfig<TContext, TEvent> | Array<TransitionConfig<TContext, TEvent>>> | Array<TransitionConfig<TContext, TEvent> & {
    delay: number;
}>;
...
export declare type TransitionsConfig<TContext, TEvent extends EventObject> = {
    [K in TEvent['type'] | BuiltInEvent<TEvent>['type']]?: string | TransitionConfig<TContext, TEvent extends {
        type: K;
    } ? TEvent : EventObject> | Array<TransitionConfig<TContext, TEvent extends {
        type: K;
    } ? TEvent : EventObject>>;
};
export declare type TransitionsDefinition<TContext, TEvent extends EventObject> = {
    [K in TEvent['type']]: Array<TransitionDefinition<TContext, TEvent extends {
        type: K;
    } ? TEvent : EventObject>>;
};
...
export declare type InvokesConfig<TContext, TEvent extends EventObject> = InvokeConfig<TContext, TEvent> | Array<InvokeConfig<TContext, TEvent>>;
...
export interface AtomicStateNodeConfig<TContext, TEvent extends EventObject> extends StateNodeConfig<TContext, never, TEvent> {
    initial?: undefined;
...
export declare type ServiceConfig = string | StateNode | StateNodeDefinition<any, any, any>;
...
export interface RaiseEvent<TContext, TEvent extends EventObject> extends ActionObject<TContext> {
    event: Event<TEvent>;
}
...
export interface SendAction<TContext, TEvent extends EventObject> extends ActionObject<TContext> {
    to: string | undefined;
    event: TEvent;
    delay?: number;
    id: string | number;
...
export interface AssignAction<TContext, TEvent extends EventObject> extends ActionObject<TContext> {
...
export interface Segment<TContext = DefaultContext, TEvent extends EventObject = EventObject> {
    /**
     * From state.
     */
    state: StateValue;
