...
var Interpreter = /** @class */ (function () {
    ...
    Interpreter.prototype.update = function (state, event) {
        ...
        this.listeners.forEach(function (listener) { return listener(state, eventObject); });
    ...
    Interpreter.prototype.exec = function (action, context, event) {
        ...
        switch (action.type) {
            ...
            case actionTypes.start: {
                var activity_1 = action
                    .activity;
                // Invoked services
                if (activity_1.type === types_1.ActionTypes.Invoke) {
                    var service = activity_1.src
                        ? activity_1.src instanceof StateNode_1.StateNode
                            ? activity_1.src
                            : typeof activity_1.src === 'function'
                                ? activity_1.src
                                : this.machine.options.services
                                    ? this.machine.options.services[activity_1.src]
                                    : undefined
                        : undefined;
                    var id_1 = activity_1.id, data = activity_1.data;
                    ...
                    if (typeof service === 'function') {
                        ...
                        var interpreter_1 = this.spawn(data
                            ? childMachine.withContext(utils_1.mapContext(data, context, event))
                            : childMachine, {
                            id: id_1,
                    ...
                    var implementation = this.machine.options && this.machine.options.activities
                        ? this.machine.options.activities[activity_1.type]
                        : undefined;
                    if (!implementation) {
                        // tslint:disable-next-line:no-console
